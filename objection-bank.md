...
| STT (e.g. Deepgram Nova-3) | $0.0048 to $0.0058/min |
...
**$0.045 to $0.05/min** (agent + STT + TTS + LLM), and **TTS is the single biggest line item**. Roughly
...
|---|---|---|---|
| **Vapi** | Managed orchestrator | $0.05/min platform fee; **$0.20 to $0.33/min** realistic with BYOK models | Fastest path for simple outbound/inbound call bots; strong Twilio integration |
...
- HIPAA on Vapi costs **+$1,000/mo**; Retell includes it. This is a real, checkable differentiator, but it cuts *against* LiveKit in deals where Retell's compliance packaging is the buying criterion.
...
| 1.2 | "What does this cost us at 50k minutes a month?" | "It's about a penny a minute." | Builds the stack: $0.01 agent + STT + TTS + LLM ≈ $0.045 to $0.05/min all-in; flags TTS as the dominant term and a lever they control. |
